...
BEGIN
   ...
   counter1 : counter
--      PORT MAP (clk=>clk, run=>run, load=>selwrite, nextkey=>nextkey,
      PORT MAP (clk=>clk, run=>run, load=>selwrite, nextkey=>nextkey,
      data=>data, valid=>keyvalid, count=>trialkey);
...
--keyvalid <= '1'; -- DES only
   ...
   decryptor : rc5_decrypt_top
      PORT MAP (clk=>clk, run=>run, ctext=>ctext, iv=>iv,
      trial_key=>trialkey(71 downto 0),
      ptext=>XLXN_28, ptext_valid=>ptextvalid, next_key=>nextkey);
   ...
   sureg1 : sureg
      PORT MAP (keyvalid=>keyvalid, match=>match, run=>run, selwrite=>selwrite, clk=>clk,
